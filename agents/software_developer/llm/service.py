...
from .prompts import format_requirement_analysis_prompt, format_project_plan_prompt, format_tech_stack_prompt, format_frontend_prompt, format_backend_prompt, format_database_prompt
...
class LLMService:
    ...
    async def get_tech_stack(self, project_description: str) -> Dict[str, Any]:
        ...
        try:
            # Format the prompt
            formatted_prompt = format_tech_stack_prompt(project_description)
            ...
            response = await openai.ChatCompletion.acreate(
                model=self.model,
                messages=[
                    {"role": "system", "content": "You are a skilled sotware developer and full stack developer."},
            ...
            self.logger.debug(f"Analysis contains {len(analysis.get('tech_stack', []))} techstacks")
    ...
    async def get_front_end(self, requirements: List[str], tech_stack: str):
        self.logger.info("generating frontend code")
        self.logger.debug(f"Requirements: {requirements}, frontend stack: {tech_stack}")

        try:
            # Format the prompt
            formatted_prompt = format_frontend_prompt(requirements, tech_stack)
            ...
            # Parse response
            analysis = self._parse_llm_response(response.choices[0].message.content)
            self.logger.info("generating frontend completed successfully")
            self.logger.debug(f"Analysis contains {len(analysis.get('features', []))} features")
    ...
    async def get_back_end(self, requirements: List[str], tech_stack: str):
        self.logger.info("generating backend code")
        self.logger.debug(f"Requirements: {requirements}, backend stack: {tech_stack}")

        try:
            # Format the prompt
            formatted_prompt = format_backend_prompt(requirements, tech_stack)
            ...
            # Parse response
            analysis = self._parse_llm_response(response.choices[0].message.content)
            self.logger.info("generating backend completed successfully")
            self.logger.debug(f"Analysis contains {len(analysis.get('features', []))} features")
    ...
    async def get_database(self, requirements: list, tech_stack: str) -> Dict[str, Any]:
        self.logger.info("generating database code")
        self.logger.debug(f"Requirements: {requirements}, database stack: {tech_stack}")

        try:
            # Format the prompt
            formatted_prompt = format_database_prompt(requirements, tech_stack)
